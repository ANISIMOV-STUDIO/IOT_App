# BREEZ Home UI/UX Improvement Plan
## "What Would Steve Say?" Edition

> "Design is not just what it looks like and feels like. Design is how it works." - Steve Jobs

---

## PHASE 1: КРИТИЧЕСКИЕ - Архитектура информации ✅

### 1.1 Устранить дублирование
- [x] **P1-1**: Убрать дублирование "ZILON ZPE-6000" → DeviceHeaderCard теперь только power toggle
- [x] **P1-2**: Объединить уведомления → только NotificationCenterCard в Global Zone
- [x] **P1-3**: Убрать "ОБЩИЕ ДАННЫЕ" label → заменён на spacing

### 1.2 Создать визуальную иерархию
- [x] **P1-4**: PRIMARY action = большая кнопка питания в центре DeviceHeaderCard
- [x] **P1-5**: Сгруппировать элементы → EnvironmentCard (Temp+Humidity+CO2)
- [x] **P1-6**: Device/Global Zone → DeviceZoneContainer визуально выделяет зону

---

## PHASE 2: ТИПОГРАФИКА - Читаемость и консистентность 🔄

### 2.1 Исправить обрезанный текст
- [x] **P2-1**: "Статистика использ..." → сокращено до "Энергия"
- [x] **P2-2**: "Охлажден..." → mode buttons теперь только иконки + Tooltip
- [ ] **P2-3**: Аудит всех текстов на overflow/ellipsis

### 2.2 Унифицировать числа и единицы
- [x] **P2-4**: Температура: всегда целые (22°)
- [x] **P2-5**: Единицы: температура °, влажность %, CO2 ppm, энергия кВт⋅ч
- [ ] **P2-6**: Создать Typography Scale и применить везде

---

## PHASE 3: ЦВЕТ И ВИЗУАЛ - Система, не хаос

### 3.1 Цветовая система
- [ ] **P3-1**: Определить semantic colors: success, warning, error, info
- [ ] **P3-2**: Уменьшить palette - максимум 3-4 accent цвета
- [ ] **P3-3**: Зелёные точки статуса - сделать subtle, не кричащими
- [ ] **P3-4**: Badge colors - красный только для errors/alerts

### 3.2 Визуальная консистентность
- [ ] **P3-5**: CO₂ gauge - редизайн в стиле остальных компонентов
- [ ] **P3-6**: Все карточки - единый border-radius, shadow, padding
- [ ] **P3-7**: Temperature dial справа - упростить или переделать

---

## PHASE 4: UX - Понятность без инструкций

### 4.1 Добавить контекст
- [ ] **P4-1**: Toggle в device card - добавить label что включает
- [ ] **P4-2**: "Быстрые действия" - добавить labels под иконки
- [ ] **P4-3**: Слайдеры Приток/Вытяжка - показать что значат значения (норма, мин, макс)
- [ ] **P4-4**: Влажность, PM2.5, VOC - добавить индикацию "хорошо/плохо"

### 4.2 Убрать пустоту
- [ ] **P4-5**: "Расписание" карточка - показать контент или убрать
- [ ] **P4-6**: Добавить meaningful empty states везде

---

## PHASE 5: LAYOUT & SPACING - Сетка и ритм

### 5.1 Grid система
- [ ] **P5-1**: Определить 8px grid и применить везде
- [ ] **P5-2**: Выровнять все карточки по сетке
- [ ] **P5-3**: Консистентные gaps между секциями

### 5.2 Карточки
- [ ] **P5-4**: Унифицировать внутренний padding карточек
- [ ] **P5-5**: Определить стандартные высоты карточек (S, M, L)

---

## PHASE 6: FOOTER & CHROME - Убрать шум

### 6.1 Footer cleanup
- [ ] **P6-1**: Погода (+5°C) - переместить в более видное место или убрать
- [ ] **P6-2**: "Синхронизировано в XX:XX" - убрать или сделать subtle
- [ ] **P6-3**: Версия v1.0.0 - убрать из основного UI (оставить в Settings)

### 6.2 Header/Sidebar polish
- [ ] **P6-4**: Аватар - добавить настоящее фото или улучшить placeholder
- [ ] **P6-5**: Sidebar hover states
- [ ] **P6-6**: Active state более выраженный

---

## PHASE 7: POLISH & DETAILS - Финальный блеск

### 7.1 Micro-interactions
- [ ] **P7-1**: Hover states для всех интерактивных элементов
- [ ] **P7-2**: Transitions/animations (subtle, 200-300ms)
- [ ] **P7-3**: Loading states

### 7.2 Accessibility
- [ ] **P7-4**: Contrast ratio check (WCAG AA minimum)
- [ ] **P7-5**: Focus states для keyboard navigation
- [ ] **P7-6**: Touch targets минимум 44x44px

---

## ПРИНЦИПЫ (проверять каждое решение)

1. **Иерархия**: Что самое важное? Это видно сразу?
2. **Простота**: Можно ли убрать? Если да - убрать.
3. **Консистентность**: Похожие вещи выглядят одинаково?
4. **Контекст**: Пользователь понимает что это без объяснений?
5. **Эмоция**: Приятно ли пользоваться? Хочется ли вернуться?

---

## PROGRESS TRACKING

Started: 2025-12-23
Current Phase: 2
Completed: 10/40 (Phase 1 done + P2-1,2,4,5)

---

## STEVE'S VOICE (напоминание)

> "Simple can be harder than complex. You have to work hard to get your thinking clean to make it simple."

> "People don't know what they want until you show it to them."

> "Details matter, it's worth waiting to get it right."
